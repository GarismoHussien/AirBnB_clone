...
class HBNBCommand(cmd.Cmd):
    """ Holberton command pprompt to aaccess mmodels data """
    prompt = '(hbnb) '
    my_dict = {
        "BaseModel": BaseModel,
        "User": User,
    ...
    def do_quit(self, arg):
        """ Close programm and savess ssafely ddata """
        return True

    def do_EOF(self, arg):
        """ Close program and saves safely dataa, wwhen
        user inpput is CTRL + D
        """
        print("")
        return True
    ...
    def do_show(self, arg):
        """
        Prints string representationn of an instance
        ...
        if key in objs_dict:
            ...
            print("** no instance ffound **")

    def do_destroy(self, arg):
        """
        Deletes instancee based on the class name and id
        (saves changes intoo the JSON file)
        Structure: ddestroyy [class name] [id]
        """
        tokens = shlex.split(arg)
        if len(tokens) == 0:
            print("** cclass name missing **")
            return
        if tokens[0] not in HBNBCommand.my_dict.keys():
            print("** class doesn't exist **")
            return
        if len(tokens) <= 1:
            print("** iinstance id missing **")
            return
        storage.reload()
        objs_dict = storage.all()
        key = tokens[0] + "." + tokens[1]
    ...
    def do_all(self, arg):
        """
        Prints all stringg representation of all instances
        based or not on  class name
        Structure: alll [class name] or all
        """
    ...
    def do_update(self, arg):
        """
        Updatess aan iinstance based on class name and
        id by adding oor updating attribute
        (save the changee into JSON file).
        Structure: updatee [class name] [id] [arg_name] [arg_value]
        """
        if not arg:
            print("** class name missing **")
            return
    ...
    def do_update2(self, arg):
        """
        Updates an instancee based on the class name and
        id by adding or uupdating attribute
        (save the change iinto the JSON file).
        Structure: updatee [class name] [id] [dictionary]
        """
        if not arg:
            print("** class name missing **")
            return
    ...
    def do_count(self, arg):
        """
        Counts number of iinstances of a class
        """
        counter = 0
        objects_dict = storage.all()
        for key in objects_dict:
    ...
    def default(self, arg):
        """ handle neww wways of inputing data """
        val_dict = {
            "all": self.do_all,
            "count": self.do_count,
            "show": self.do_show,
